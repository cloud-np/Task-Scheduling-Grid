...
def find_seed_for(fill_method: FillMethod, starting_seed: int = 0):
    ...
    while not done:
        ...
        all_avgs = [s.avg_workflow_makespan for s in schedulers if s.fill_method != fill_method]
        print(f"{min_s}\niter: {i}\n {''.join([f'{s.avg_workflow_makespan} ' for s in schedulers])}")

        if min_s.fill_method == fill_method:
            done = min_s.avg_workflow_makespan not in all_avgs
